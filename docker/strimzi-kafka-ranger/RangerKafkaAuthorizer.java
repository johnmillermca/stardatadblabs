...
public class RangerKafkaAuthorizer implements Authorizer {
  ...
  private Collection<RangerAccessResult> callRangerPlugin(List<RangerAccessRequest> rangerRequests) {
    // KRaft combined-mode fix: in combined controller+broker mode Kafka creates two
    // RangerKafkaAuthorizer instances. The second instance finds rangerPlugin already
    // initialized (static field) and skips the synchronized block in configure(), leaving
    // its own auditHandler (instance field) null. Lazily initialize it here so that
    // broker-role authorization calls do not NPE on auditHandler.flushAudit().
